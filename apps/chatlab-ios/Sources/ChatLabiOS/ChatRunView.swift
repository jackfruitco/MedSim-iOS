...
public struct ChatRunView: View {
  ...
  private func compactMessengerPanel(layoutMode: ChatRunLayoutMode, chromeMode: ChatRunChromeMode)
    -> some View
  {
    Group {
      if store.showsInitialGenerationFailureScreen {
  ...
  private var regularFailureBanners: some View {
    VStack(spacing: 10) {
      ...
      if let failure = store.simulationFailureText,
        store.showsInitialGenerationFailureScreen == false
      {
        failureBanner(
  ...
  private var disconnectedIndicator: some View {
    if isDisconnectedOrDegraded {
      Label(
        isConnectionFailed ? "Offline" : "Reconnecting",
  ...
  private func runHeader(layoutMode: ChatRunLayoutMode) -> some View {
    ...
    .clipShape(
      RoundedRectangle(cornerRadius: layoutMode == .padWorkspace ? 18 : 0, style: .continuous))
  }
      ...
      HStack(spacing: layoutMode == .padWorkspace ? 10 : 8) {
        ForEach(store.conversations) { conversation in
          Button {
            ...
            .font(
              tabFont(for: layoutMode).weight(
                store.activeConversationID == conversation.id ? .semibold : .regular)
            )
  ...
  private func messageTimeline(layoutMode: ChatRunLayoutMode, chromeMode: ChatRunChromeMode)
    -> some View
  {
      ...
      ScrollView {
        VStack(spacing: 8) {
          ...
          if store.hasMoreByConversation[store.activeConversationID ?? -1] == true {
            ...
            .padding(
              .horizontal, layoutMode == .padWorkspace ? 0 : horizontalInset(for: layoutMode))
          }
  ...
  private func submitOrdersPanel(compact: Bool) -> some View {
    VStack(alignment: .leading, spacing: 8) {
      if !toolsStore.stagedOrders.isEmpty {
        Text(
          "\(toolsStore.stagedOrders.count) order\(toolsStore.stagedOrders.count == 1 ? "" : "s") ready"
        )
        .font(.caption)
        .foregroundStyle(.secondary)
      }
...
extension String {
  fileprivate var containsMarkdownSyntaxHint: Bool {
    contains("**")
      || contains("### ")
      || contains("\n- ")
      || hasPrefix("- ")
    ...
    if items.isEmpty {
      VStack(alignment: .leading, spacing: 6) {
        Text("Activity will show recovery and simulation updates here.")
          .font(.footnote.weight(.semibold))
        Text(
          "Manual refreshes, reconnect recovery, feedback generation changes, and patient result updates appear in this feed."
        )
        .font(.footnote)
        .foregroundStyle(.secondary)
      }
...
private struct SimulationFeedbackRows: View {
  ...
  var body: some View {
    if rows.isEmpty {
      ...
      let treatmentPlanField = fields.first {
        normalizedFeedbackKey(for: $0.label) == "treatment plan"
      }
